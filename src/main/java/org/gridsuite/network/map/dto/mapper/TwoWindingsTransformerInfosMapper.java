...
public final class TwoWindingsTransformerInfosMapper extends BranchInfosMapper {
    ...
    private static TwoWindingsTransformerFormInfos toFormInfos(Identifiable<?> identifiable) {
        ...
        builder.selectedOperationalLimitsGroupId1(twoWT.getSelectedOperationalLimitsGroupId1().orElse(null));
        builder.selectedOperationalLimitsGroupId2(twoWT.getSelectedOperationalLimitsGroupId2().orElse(null));
    ...
    private static TwoWindingsTransformerTooltipInfos toTooltipInfos(Identifiable<?> identifiable, Double dcPowerFactor) {
        ...
        twoWindingsTransformer.getSelectedOperationalLimitsGroup1().ifPresent(limitsGrp -> limitsGrp.getCurrentLimits().ifPresent(limits -> builder.currentLimits1(toMapDataCurrentLimits(limits,
                limitsGrp.getId(), ElementInfos.InfoType.TOOLTIP))));
        twoWindingsTransformer.getSelectedOperationalLimitsGroup2().ifPresent(limitsGrp -> limitsGrp.getCurrentLimits().ifPresent(limits -> builder.currentLimits2(toMapDataCurrentLimits(limits,
                limitsGrp.getId(), ElementInfos.InfoType.TOOLTIP))));
